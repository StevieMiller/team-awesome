...
function init() {
  ...
  .then((answers) => {
    if (answers.role === 'Manager') {
      inquirer.prompt(qsMgr)
      .then(managerInfo => {
        const manager = new Manager (answers.name, answers.id, answers.email, managerInfo.officeNum)
        employees.push(manager)
        askAgain();
      }) 
    } else if (answers.role === 'Engineer') {
      inquirer.prompt(qsEng)
      .then(engineerInfo => {
        const engineer = new Engineer (answers.name, answers.id, answers.email, engineerInfo.github)
        employees.push(engineer)
        askAgain();
      })
    } else if (answers.role === 'Intern') {
      inquirer.prompt(qsInt)
      .then(internInfo => {
        const intern = new Intern (answers.name, answers.id, answers.email, internInfo.school)
